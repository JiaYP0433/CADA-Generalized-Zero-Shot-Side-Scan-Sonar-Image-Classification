...
from .utils import VGGEncoder, VGGDecoder


class SonarNoiseWCT(nn.Module):
    def __init__(self, mode='none', noise_scale=0.7):
        super(SonarNoiseWCT, self).__init__()
        style_mode = 'liwct'
        wct_path = 'datasets/transfer/params/{}/weight.pth'.format(style_mode)

        self.e1 = VGGEncoder(mode=style_mode, level=1)
        self.d1 = VGGDecoder(style_mode=style_mode, level=1)
        self.e2 = VGGEncoder(mode=style_mode, level=2)
        self.d2 = VGGDecoder(style_mode=style_mode, level=2)
        self.e3 = VGGEncoder(mode=style_mode, level=3)
        self.d3 = VGGDecoder(style_mode=style_mode, level=3)
        self.e4 = VGGEncoder(mode=style_mode, level=4)
        self.d4 = VGGDecoder(style_mode=style_mode, level=4)

        self.load_state_dict(torch.load(wct_path))
        self.noise_scale = noise_scale

    def forward(self, cont_img, styl_img, cont_seg=None, styl_seg=None):
        self.__compute_label_info(cont_seg, styl_seg)
        sF4, sF3, sF2, sF1 = self.e4.forward_multiple(styl_img)
        cF4, cpool_idx = self.e4(cont_img)
        sF4 = sF4.data.squeeze(0)
        cF4 = cF4.data.squeeze(0)
        csF4 = self.__feature_wct(cF4, sF4, cont_seg, styl_seg)
        Im4 = self.d4(csF4, pool_dix=cpool_idx, noise_scale=self.noise_scale)
        cF3, cpool_idx = self.e3(Im4)
        sF3 = sF3.data.squeeze(0)
        cF3 = cF3.data.squeeze(0)
        csF3 = self.__feature_wct(cF3, sF3, cont_seg, styl_seg)
        Im3 = self.d3(csF3, pool_dix=cpool_idx, noise_scale=self.noise_scale)
        cF2, cpool_idx = self.e2(Im3)
        sF2 = sF2.data.squeeze(0)
        cF2 = cF2.data.squeeze(0)
        csF2 = self.__feature_wct(cF2, sF2, cont_seg, styl_seg)
        Im2 = self.d2(csF2, pool_dix=cpool_idx, noise_scale=self.noise_scale)
        cF1 = self.e1(Im2)
    ...
    def __compute_label_info(self, cont_seg, styl_seg):
        if cont_seg is None or styl_seg is None:
            return
